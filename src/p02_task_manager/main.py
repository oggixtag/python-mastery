...
# Fonction pour charger les taches depuis un fichier JSON
def charger_fichier(chemin_fichier):
    # Charge les données depuis un fichier JSON.

    # 1. Gestion du cas où le fichier n'existe pas encore
    if not os.path.exists(chemin_fichier):
        # Retourne une liste vide
        return []
    
    # 2. Lecture du fichier si tout est correct
...
def sauvegarde(ma_liste, chemin_fichier):
...
def main():
    ...
    base_dir = os.path.dirname(__file__) #p02_repertoire
    parent = os.path.dirname(base_dir) #src
    parent = os.path.dirname(parent) #python-mastery
    data_dir = os.path.normpath(os.path.join(parent, 'data'))
    ...
    #On vérifie si le fichier JSON existe ou pas
    taches = charger_fichier(fichier_taches)
    ...
    while True:
        ...
        if action == 'r':
            ...
            sauvegarde(taches, fichier_taches)
